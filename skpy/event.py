...
class SkypeMessageEvent(SkypeEvent):
    ...
    attrs = SkypeEvent.attrs + ("msgId", "msgContent", "msgFrom", "resource")

    @classmethod
    def rawToFields(cls, raw : dict={}):
        fields = super(SkypeMessageEvent, cls).rawToFields(raw)
        res = raw.get("resource", {})
        fields["msgId"] = int(res.get("id")) if "id" in res else None
        fields["msgContent"] = res.get("content")
        fields["msgFrom"] = res.get("imdisplayname")
        fields["resource"] = res
...
class SkypeChatUpdateEvent(SkypeEvent):
    ...
    attrs = SkypeEvent.attrs + ("chatId", "horizon", "resource")
