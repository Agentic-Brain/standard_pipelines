...
class SkypeConnection(SkypeObj):
    ...
    def externalCall(cls, method, url, codes=(200, 201, 204, 207), **kwargs):
        ...
        print(f"(ext) [{method}] {url} {resp.status_code}")
        if os.getenv("SKPY_DEBUG_HTTP"):
    ...
    API_DIRECTORY = "https://search.skype.com/v2.0/search/"
    ...
    attrs = ("user", "userId", "tokenFile", "connected", "guest")
    ...
    def __call__(self, method, url, codes=(200, 201, 202, 204, 207), auth=None, headers=None, **kwargs) -> requests.Response:
        """
    ...
    def getUser(self):
        ...
        """
        self.user = self("GET", "{0}/users/self/profile".format(self.API_USER),
                           auth=self.Auth.SkypeToken).json()
        self.userId = self.user.get("username")
