...
class SkypeUser(SkypeObj):
    id: str
    name: 'SkypeUser.Name'
    location: 'SkypeUser.Location'
    language: Optional[str]
    avatar: Optional[str]
    mood: Optional['SkypeUser.Mood']
    ...
    def invite(self, greeting=None) -> requests.Response:
        """
        ...
        print("INVITING:", greeting)
        prefix = "28" if isinstance(self, SkypeBotUser) else "8"
        return self.skype.conn("POST", "{0}/users/{1}/contacts".format(SkypeConnection.API_CONTACTS, self.skype.userId),
...
class SkypeContacts(SkypeObjs):
    ...
    def __init__(self, skype=None):
        ...
        self.contactIds : list[str] = []
        self.groups : dict[str, SkypeContactGroup] = {}
    ...
    max_search_interval : timedelta = timedelta(seconds=10)
    last_search_time : datetime = None
    @SkypeUtils.cacheResult
    def search(self, query, search_guid : str = None):
        """
        ...
        if self.last_search_time is not None:
            delta : timedelta = datetime.now(UTC) - self.last_search_time
            if (delta < self.max_search_interval):
                remaining_seconds = self.max_search_interval.total_seconds() - delta.total_seconds()
                print('waiting for', remaining_seconds, 'seconds')
                time.sleep(remaining_seconds)
            else:
                print('not waiting, delta=', delta)
        else:
            print('no last search time')
        self.last_search_time = datetime.now(UTC)

        request_params = {"searchstring": query, "requestId": "0"}
        if (search_guid is not None):
            request_params["sessionId"] = search_guid
        
        conn : SkypeConnection = self.skype.conn
        response : requests.Response = conn("GET", SkypeConnection.API_DIRECTORY,
                                  auth=SkypeConnection.Auth.SkypeToken,
        ...
        results = content_body_json.get("results", [])
        

        parsed_results = []
        for item in results:
            parsed_results.append(SkypeUser.fromRaw(self.skype, item.get("nodeProfileData", {})))
