...
from standard_pipelines.api.gmail.models import GmailCredentials, GoogleCredentials
...
def login_google():
    ...
    current_app.logger.debug(f"Google Client ID: {current_app.config['GMAIL_CLIENT_ID'][:5]}...")
    if not current_app.config.get('GMAIL_CLIENT_ID'):
        current_app.logger.error("GMAIL_CLIENT_ID is not configured")
    ...
    try:
        auth_redirect = oauth.google.authorize_redirect(
            redirect_uri,
            scope=current_app.config['GMAIL_SCOPES'].split()
        )
        current_app.logger.debug(f"Authorization URL generated successfully")
        return auth_redirect
        ...
        try:
            ...
            if existing_creds:
                ...
                credentials = GoogleCredentials(
                    client_id=client_id,
                    google_client_id=current_app.config['GMAIL_CLIENT_ID'],
                    google_client_secret=current_app.config['GMAIL_CLIENT_SECRET'],
