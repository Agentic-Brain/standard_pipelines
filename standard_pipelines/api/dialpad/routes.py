...
def manage_dialpad_credentials(client_id: str):
    ...
    try:
        ...
        existing_credentials = DialpadCredentials.query.filter_by(client_id=client_uuid).first()
        ...
        current_app.logger.exception(f"Unknown error with managing Dialpad credentials: {e}")
        return jsonify({'error': f"Unknown error with managing Dialpad credentials: {e}"}), 500
